...
{
	/* LOCAL FIX (pistorm-atari STBOX): MOVE16 is 68040-only, but the
	 * jump table installs this handler for every CPU type and, unlike
	 * its neighbours, it carried no runtime guard. On a 68000 it then
	 * EXECUTES, swallowing its extension word, where the silicon would
	 * raise line-F - and TOS 1.04's AES is built on line-F trampolines
	 * (field failure: any desktop click -> stream derailed -> jump via
	 * a junk dispatch entry -> two bombs). Trap like the real chip. */
	if(!CPU_TYPE_IS_040_PLUS(CPU_TYPE))
	{
		m68ki_exception_1111();
		return;
