...
static void do_interrupt(int nr)
{
	...
	/* Atari ST interrupt routing:
	 *   IPL2 = HBL autovector, IPL4 = VBL autovector, IPL6 = MFP vectored.
	 * Only the MFP owns the interrupt-acknowledge cycle. IACKing IPL2/IPL4 can
	 * put arbitrary bus data into pistorm_iack_vector and dispatch a bogus
	 * exception (for example EmuTOS panic "Exception number 125" / 0x7D). */
	if (nr == 6)
		intlev_ack(nr);
	else
