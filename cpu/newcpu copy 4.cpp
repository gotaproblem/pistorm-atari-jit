...
static inline void atari_serial_irq_poll(void)
{
	...
	/*
	 * Serial IRQ isolation mode: no IPL pthread.  Sample the hardware pins in
	 * the CPU path, then let do_specialties() call do_interrupt(), which does
	 * the real bus IACK and builds the exception frame.
	 */
	...
	if (currprefs.cachesize)
		...
		set_special(SPCFLAG_INT | SPCFLAG_DOINT);
#endif
}
...
static void MakeFromSR_x(int t0trace)
{
	...
	/* Keep the UAE CPU mask and the PiStorm IRQ gate mask in lockstep.
	 * The JIT wake path tests regs.intmask, while intlev()/ipl_task gate on
	 * g_irq_mask. If either side is stale, STOP can miss a valid wake IRQ. */
		int newm = sr_intmask;
		if (regs.intmask != newm || g_irq_mask != newm)
		{
			int old_irq_mask = g_irq_mask;
			if (g_irq <= old_irq_mask && g_irq > newm)
			{
			//set_special(SPCFLAG_DOINT);
			if (!currprefs.cachesize)
			{
...
static int effective_interrupt_vector(const int nr, const int vector_nr)
{
	...
	if (!is_interrupt_exception_vector(nr) ||
		pistorm_iack_vector == 0xFFFF || pistorm_iack_vector == 0xFFFE)
		return vector_nr;
...
static void do_interrupt(int nr)
{
	...
	/* Match the old Musashi contract: acknowledge every interrupt level on the
	 * real bus. IPL2/IPL4 remain autovectors; IPL6/MFP uses the returned vector
	 * for the vector-table lookup and stacked format/vector word. */
	intlev_ack(nr);
	Exception(nr + 24);
	...
	else
	{
		...
		if (spcflags & (SPCFLAG_DOINT | SPCFLAG_INT))
		{
			...
			if (irq > regs.intmask) {
				...
				/* Masked interrupts are not acknowledged by the CPU. Keep the
				 * pending level visible so a later SR mask change or higher
				 * hardware level can wake STOP through the normal path. */
				regs.ipl_pin = irq;
			}
