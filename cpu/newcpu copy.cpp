...
kludge_me_do:
	...
	// Exception_normal(), replacing line 3428 (newpc = x_get_long(regs.vbr + 4*vector_nr);)
	extern uae_u16 pistorm_iack_vector;
	int level = nr - 24;
	int eff_vec;
	if (interrupt && level == 6 &&
		pistorm_iack_vector != 0xFFFF && pistorm_iack_vector != 0xFFFE)
		eff_vec = pistorm_iack_vector & 0xFF;   // MFP, vectored
	else if (interrupt && pistorm_iack_vector == 0xFFFE)
		eff_vec = 24;                            // spurious -> vector 24
	else
		eff_vec = vector_nr;                     // autovector (levels 2/4) or non-interrupt
...
static void do_interrupt (int nr)
{
	...
	*/
	...
	else if (nr == 6) { // MFP definately needs real bus ack
		intlev_ack (nr); 
		Exception (pistorm_iack_vector & 0xFF);
	}
	else { // spurious?
	...
	//g_irq_mask = (regs.sr >> 8) & 7; // important - sets mask check for ipl_task()
	//g_irq = 0;
#ifdef ATARI_VBS_MON
	...
	if (spcflags & SPCFLAG_MMURESTART) {
		...
		//{
			...
			if (spcflags & SPCFLAG_DOINT) {
				...
				if ((regs.ipl_pin > regs.intmask) || (regs.ipl_pin == 7 && regs.ipl_pin > regs.lastipl)) {//} || (g_irq == 7 && g_irq > regs.lastipl)) {
...
insretry:
				...
				if (!currprefs.cpu_cycle_exact) {
					...
					regs.instruction_cnt++;
					regs.ipl[0] = regs.ipl_pin;
					if (regs.spcflags) {// || time_for_interrupt ()) { // cryptodad ints use g_irq
						if (do_specialties(0)) {
							STOPTRY;
							return;
						}
...
void m68k_go (int may_quit)
{
	...
	for (;;) {
		...
		if (!regs.halted) {
			...
			if (ab == nullptr || ab == &pistorm_dummy_bank || (!currprefs.cpu_compatible && !valid_address(pc, 2)) || (pc & 1)) { //cryptodad
...
bool cpureset (void)
{
	...
	{ extern void cpu_pulse_reset(void); cpu_pulse_reset(); }
