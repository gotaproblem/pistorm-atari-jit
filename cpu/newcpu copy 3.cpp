...
#endif
	if (changed || currprefs.cpu_model != changed_prefs.cpu_model || currprefs.fpu_model != changed_prefs.fpu_model || currprefs.fpu_revision != changed_prefs.fpu_revision || currprefs.mmu_model != changed_prefs.mmu_model || currprefs.mmu_ec != changed_prefs.mmu_ec || currprefs.cpu_data_cache != changed_prefs.cpu_data_cache || currprefs.int_no_unimplemented != changed_prefs.int_no_unimplemented || currprefs.fpu_no_unimplemented != changed_prefs.fpu_no_unimplemented || currprefs.cpu_compatible != changed_prefs.cpu_compatible || currprefs.cpu_cycle_exact != changed_prefs.cpu_cycle_exact || currprefs.cpu_memory_cycle_exact != changed_prefs.cpu_memory_cycle_exact || currprefs.fpu_mode != changed_prefs.fpu_mode)
	{
		cpu_prefs_changed_flag |= 1;
	}
	if (changed || currprefs.m68k_speed != changed_prefs.m68k_speed || currprefs.m68k_speed_throttle != changed_prefs.m68k_speed_throttle || currprefs.cpu_clock_multiplier != changed_prefs.cpu_clock_multiplier || currprefs.reset_delay != changed_prefs.reset_delay || currprefs.cpu_frequency != changed_prefs.cpu_frequency)
	{
...
{
	regs.sr = ((regs.t1 << 15) | (regs.t0 << 14) | (regs.s << 13) | (regs.m << 12) | (regs.intmask << 8) | (GET_XFLG() << 4) | (GET_NFLG() << 3) | (GET_ZFLG() << 2) | (GET_VFLG() << 1) | GET_CFLG());
}
...
static void MakeFromSR_x(int t0trace)
{
	...
	if (g_irq_mask != ((regs.sr >> 8) & 7))
	{
		int newm = (regs.sr >> 8) & 7;
		if (g_irq_mask == 0)// || newm == 0)
		{
			static int n;
			if (n++ < 200)
				fprintf(stderr, "[MASK] %d->%d  sr=%04X  s=%d  pc=%08X\n",
						g_irq_mask, newm, regs.sr, regs.s, (unsigned)m68k_getpc());

			//regs.intmask = newm = 3; // cryptodad - will this work?
		}

		if (g_irq <= g_irq_mask && g_irq > newm)
		{
			//set_special(SPCFLAG_DOINT);
			if (!currprefs.cachesize)
			{
...
{
	extern addrbank pistorm_dummy_bank; // cryptodad
	uae_u32 newpc;
	...
	if (!currprefs.cpu_compatible)
	{
		...
		if (!ab || ab == &pistorm_dummy_bank || (ab->flags & ABFLAG_IO))
		{
...
#endif
	// extern volatile uint8_t g_irq;
	// regs.ipl_pin = regs.intmask;//g_irq;//real_irq;//intlev();
	ipl_fetch_now();
	fill_prefetch();
	exception_check_trace(nr);
}
...
void REGPARAM2 Exception(int nr)
{
	...
	// else
	//	ExceptionX(nr, g_buserr_addr, m68k_getpc());
}
...
static void do_interrupt(int nr)
{
	...
	extern volatile uint8_t g_irq;
	extern volatile uint8_t g_irq_mask;
	int bad_iack = 0;  /* set when IACK returned an out-of-range vector (drop it) */
#ifdef DEBUGGER
	if (debug_dma)
		record_dma_event(DMA_EVENT_CPUIRQ);
#endif
#if (0) // ATARI does not need this
...
//#define IRQ_PROFILE
	...
	else if (pistorm_iack_vector == 0xFFFE)
	{
		...
		uae_u8 vec = pistorm_iack_vector & 0xFF;
		/* A legitimate vectored Atari interrupt is an MFP vector in 0x40..0x4F.
		 * Anything else is a malformed IACK read (observed: a transient FF7D on
		 * a level-6 IACK that should have been FF45). We must NOT dispatch it:
		 * a real vector (0x7D) runs EmuTOS's unhandled-vector stub -> panic 125,
		 * and redirecting to the spurious vector (24) ALSO panics (EmuTOS has no
		 * benign handler for 24 either). The only safe thing is to DROP the
		 * interrupt entirely: take no exception, leave the mask alone, fall
		 * through to the g_irq=0 re-arm at the end so the device re-asserts and
		 * the next IACK reads the correct 0x45. A single dropped MFP tick is
		 * invisible; a panic is not. (Read-side cause of 0x7D handled separately.) */
		if (vec < 0x40 || vec > 0x4F)
		{
			fprintf(stderr, "[BADVEC->DROP] nr=%u iack_word=%04X vec=%02X pc=%08X\n",
					nr, (unsigned)pistorm_iack_vector, (unsigned)vec,
					(unsigned)m68k_getpc());
			bad_iack = 1;
		}
		else
	...
	/* Skip the mask-raise + take bookkeeping when we dropped a bad IACK; just
	 * re-arm (g_irq=0) at the end so the interrupt can be re-sampled cleanly. */
	if (!bad_iack)
	{
	/* Exception() raises the SR mask ONLY for autovectors (its internal test is
	 * nr in [24,32)). The MFP is a VECTORED interrupt (vector 0x45, i.e. >= 32),
	 * so Exception() left the mask untouched and the handler would run at the
	 * pre-interrupt level (e.g. 3 from a STOP #$2300) -> re-entered before it
	 * clears the MFP in-service bit -> stack/frame corruption -> EmuTOS panic
	 * (exception 125, sr=2300). Raise the mask to the interrupt LEVEL here.
	 * regs.intmask is the master shadow MakeSR() rebuilds regs.sr from, so set
	 * it (not regs.sr alone, or the next MakeSR would undo it); also set the sr
	 * bits so any immediate reader is coherent. For autovectors this is a no-op
	 * (Exception already set intmask = nr-24 == nr here). */
	regs.intmask = nr;
	regs.sr = (regs.sr & ~0x0700) | (nr << 8);

	/* diag: g_irq_mask is what intlev() actually gates on; confirm it now
	 * agrees with SR (this line should stop printing once the fix holds). */
	//if (g_irq_mask != ((regs.sr >> 8) & 7))
	//	printf ("[DO_INT] HMMM g_irq_mask %d not equal to SR %d\n", g_irq_mask, ((regs.sr >> 8) & 7));
	g_irq_mask = ((regs.sr >> 8) & 7);//regs.intmask;
	} /* end if(!bad_iack) */
	...
	{ 
		...
		/* MFP is a VECTORED interrupt: its vector (>= 0x40) falls outside
		 * Exception_normal's autovector test (nr in [24,32)), so the mask was
		 * NOT raised there. Raise it here to the interrupt LEVEL (nr) exactly
		 * as Exception_normal does for autovectors (regs.intmask = nr-24).
		 * Without this the MFP handler runs unmasked, gets re-entered before it
		 * clears the MFP in-service bit, the ISR bit stays stuck (timer tick
		 * stops -> mfp count falls to 0) and the nesting leaves intmask wedged. */
	...
	/* Vectored Atari/MFP interrupts enter Exception() with the device-supplied
	 * vector number, not nr+24. Exception_normal() only raises SR's interrupt
	 * mask for autovector-style nr+24 calls, so do it here for all IACK
	 * outcomes after the old SR has been stacked. */
	//regs.intmask = nr;
	//g_irq_mask = regs.intmask;
	//MakeSR();

	
#define ATARI_VBS_MON
#ifdef ATARI_VBS_MON
	/* VBL = level-4 autovector = vector 0x1C. Count, print once/sec, reset. */
	{
		...
		{
			t = now;
			printf("[INTS/s] ipl2 %u,  vbl %u, mfp %u, others %u - g_irq_mask %d, g_irq %d, intmask %d, ipl_pin %d\n", 
				irq2, vbl4, mfp6, others, g_irq_mask, g_irq, regs.intmask, regs.ipl_pin);
			fflush(stdout);
...
{
	return 0; // cryptodad ints use g_irq
...
void doint()
{
	...
	if (regs.ipl_pin != ipl)
	{
		...
		if (currprefs.cs_ipldelay && m68k_accurate_ipl && regs.ipl_pin >= 0 && ipl >= 0 && (((regs.ipl_pin & 1) && !(ipl & 1)) || ((regs.ipl_pin & 2) && !(ipl & 2)) || ((regs.ipl_pin & 4) && !(ipl & 4))))
		{
...
static int do_specialties(int cycles)
{
	...
	else
	{
		...
		if (spcflags & (SPCFLAG_DOINT | SPCFLAG_INT))
		{
			...
			/* Wake a STOPped CPU. STOP (#imm) sets regs.stopped and parks the
			 * CPU until an interrupt of level > the new SR mask arrives. We get
			 * here precisely because intlev() saw g_irq > regs.intmask and
			 * raised SPCFLAG_DOINT, i.e. a qualifying wake-up interrupt is
			 * pending. Nothing on this Atari path cleared regs.stopped, so the
			 * CPU took the handler but stayed flagged stopped and re-parked on
			 * the STOP forever (observed: pinned on STOP #$2300 at imask=3 while
			 * MFP/IPL6 fired into the void). m68k_resumestopped() both advances
			 * the PC past the STOP opcode AND clears regs.stopped, so after the
			 * interrupt's RTE execution continues at the instruction following
			 * STOP rather than re-parking on it. (Plain m68k_unset_stop() would
			 * clear the flag but leave PC on the STOP -> immediate re-stop.) */
			if (regs.stopped)
				m68k_resumestopped();
			...
			// || (regs.ipl_pin == 7 && regs.ipl_pin > regs.lastipl)) {

				do_interrupt (regs.ipl_pin);
			}

			regs.lastipl = regs.ipl_pin;
		}
/*
		if (spcflags & SPCFLAG_INT)
		{
...
#if defined(JIT_HAS_BUS_ERROR_RECOVERY)
/* Number of consecutive identical faults (same PC + same fault address) that
 * counts as a runaway storm. A guest whose vector handler is missing or wild
 * can otherwise re-fault the same instruction forever, flooding the host and
 * pinning a core. Tune freely. */
#ifndef JIT_FAULT_STORM_LIMIT
#define JIT_FAULT_STORM_LIMIT 64
#endif

/* Last-resort recovery for an m68k_exception that escaped the JIT dispatch
 * loop on a path with no local TRY/CATCH. The usual culprit is:
 *   do_specialties() -> do_interrupt() -> Exception() -> a memory access that
 *   bus-errors -> hardware_exception2() -> THROW() while jit_in_compiled_code
 *   was false. With no handler on the C++ stack, that throw reaches
 *   std::terminate and aborts the host (the "terminate called after throwing
 *   an instance of 'm68k_exception'" crash). Here we instead take the fault
 *   properly so the guest's own handler runs; if taking it double-faults, the
 *   CPU is halted rather than the host process killed. A circuit breaker stops
 *   runaway fault storms. */
static void jit_recover_from_fault(int vec)
{
	static uae_u32 last_pc = 0xffffffffu, last_fault = 0xffffffffu;
	static int repeat = 0;

	jit_in_compiled_code = false;

	uae_u32 pc = m68k_getpc();
	uae_u32 fault = last_fault_for_exception_3;

	if (pc == last_pc && fault == last_fault)
	{
		if (++repeat >= JIT_FAULT_STORM_LIMIT)
		{
			write_log(_T("JIT: fault storm vec=%d pc=%08x fault=%08x - halting CPU\n"),
					  vec, pc, fault);
			repeat = 0;
			last_pc = last_fault = 0xffffffffu;
			cpu_halt(CPU_HALT_DOUBLE_FAULT);
			return;
		}
	}
	...
	TRY(prb_rec)
	{
		Exception(vec); /* stack frame + vector the fault */
	}
	CATCH(prb_rec)
	{
		cpu_halt(CPU_HALT_BUS_ERROR_DOUBLE_FAULT);
...
static void m68k_run_jit(void)
{
	...
	for (;;)
	{
		...
		__try
		{
			...
			/* Catch any m68k_exception that escapes the dispatch loop on a
			 * non-compiled path (e.g. do_specialties -> do_interrupt ->
			 * Exception -> bus error -> THROW). Without this the throw unwinds
			 * past m68k_go into std::terminate and aborts the host. */
			try
			{
#endif
				for (;;)
...
// printf ("here 24\n");
			...
			{
				/* Guest fault escaped the dispatch loop. Take it properly,
				 * then fall through: the outer for(;;) re-arms the setjmp
				 * site and re-enters dispatch, so we keep running. */
				jit_recover_from_fault((int)e);
			}
#endif
#ifdef USE_STRUCTURED_EXCEPTION_HANDLING
		}
		...
		TRY(prb)
		{
			...
			insretry:
				...
				else
				{
					...
					regs.ipl[0] = regs.ipl_pin;
					if (regs.spcflags)
					{ // || time_for_interrupt ()) { // cryptodad ints use g_irq
						if (do_specialties(0))
						{
							STOPTRY;
							return;
...
static void m68k_run_2ce()
{
	...
	while (!exit)
	{
		...
		{
			bus_error();
			/* cryptodad don't check for ints after bus error
			ipl_fetch_now();
...
void m68k_run(void)
{
	...
	run_func = currprefs.cpu_cycle_exact && currprefs.cpu_model <= 68010 ? m68k_run_1_ce : currprefs.cpu_compatible && currprefs.cpu_model <= 68010 ? m68k_run_1
																					   :
#ifdef JIT
																					   ...
																					   currprefs.cpu_model < 68020 ? m68k_run_2_000
																												   : m68k_run_2_020;
...
void m68k_go(int may_quit)
{
	...
	for (;;)
	{
		...
		if (!regs.halted)
		{
			...
			if (ab == nullptr || ab == &pistorm_dummy_bank || (!currprefs.cpu_compatible && !valid_address(pc, 2)) || (pc & 1))
			{ // cryptodad
...
bool cpureset(void)
{
	...
	/* Atari: the 68k RESET instruction must pulse the real hardware reset
	 * line (MFP/GLUE/DMA/FDC/sound) WITHOUT resetting the CPU — exactly what
	 * the Musashi path did via cpu_pulse_reset(). custom_reset_cpu() below
	 * only resets Amiga custom chips, so without this the real Atari
	 * peripherals never get reset and EmuTOS spins re-reading them. */
	{
