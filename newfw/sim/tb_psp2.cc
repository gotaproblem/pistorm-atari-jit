...
static p_pistorm__psp2__core top;
...
struct Mon {
    ...
    int  rd_oe_clocks;
    int  ga_run, ga_min;                 /* '373 gate width tracking */
    void reset() { *this = Mon{}; ga_min = 999; }
} mon;
...
static void monitors(void)
{
    ...
    /* FW 0x24: every '373 gate pulse must be >=3 clocks wide */
    {
        bool any = B(top.p_ltch__a__0) || B(top.p_ltch__a__16) ||
                   B(top.p_ltch__d__wr__u);
        if (any) mon.ga_run++;
        else { if (mon.ga_run && mon.ga_run < mon.ga_min)
                   mon.ga_min = mon.ga_run;
               mon.ga_run = 0; }
    }
}
...
int main(void)
{
    ...
    /* =========== T1: word write, ordering + hold ================= */
    {
        ...
        CHECK(mon.ga_min >= 3 && mon.ga_min != 999,
              "T1: every '373 gate pulse >=3 clocks (375ns)");
    }
    ...
    /* =========== T9: CSR read - FWREV + flags ===================== */
    {
        ...
        CHECK((v & 0xFF) == 0x2A, "T9: FWREV reads 0x2A");
    ...
    /* =========== T11: debug page - cycle counters + fight ========= */
    {
        ...
        pi_rd_pulse(0);
        CHECK((v2 & 0xFF) == 0x2A, "T11: normal page restored (FWREV)");
    }
