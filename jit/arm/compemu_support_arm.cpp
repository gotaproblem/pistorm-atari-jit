...
void compile_block(cpu_history* pc_hist, int blocklen, int totcycles)
{
    if (cache_enabled && compiled_code && currprefs.cpu_model >= 68020) {
        ...
        /* PiStorm: record the 4KB pages this block came from, so a later
         * ST-RAM write to any of them drives pistorm_smc() -> cache_invalidate(). */
        {
            extern void pistorm_mark_code(uaecptr);
            for (int _k = 0; _k < blocklen; _k++)
                pistorm_mark_code((uaecptr)((uintptr)pc_hist[_k].location
        ...
        if (bi->status != BI_INVALID) {
            ...
            Dif(bi->count >= 0 && bi->status != BI_NEED_RECOMP) { // ATARI fix
