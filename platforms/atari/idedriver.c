...
/* Disk translation - LBA only.
   Returns a byte offset into the image file, or -1 on error. */
...
static void cmd_readsectors_complete(struct ide_taskfile *tf)
{
  ...
  if (d->offset == -1 ||  lseek(d->fd, d->offset, SEEK_SET) == -1) {
    tf->status |= ST_ERR;
    tf->status &= ~ST_DSC;
    tf->error |= ERR_IDNF;
    /* return null data */
...
static void cmd_verifysectors_complete(struct ide_taskfile *tf)
{
  ...
  /* 0 = 256 sectors */
  d->length = tf->count ? tf->count : 256;
  if (d->offset == -1 || lseek(d->fd, d->offset + (d->length - 1) * 512, SEEK_SET) == -1) {
    tf->status &= ~ST_DSC;
    tf->status |= ST_ERR;
    tf->error |= ERR_IDNF;
  }
...
{
  /* Report the failing LBA address back in the taskfile registers (LBA mode only) */
  off_t lba = d->offset / 512;   /* d->offset is a byte offset at this point */

  d->controller->lba1 =  lba        & 0xFF;
  d->controller->lba2 = (lba >>  8) & 0xFF;
  d->controller->lba3 = (lba >> 16) & 0xFF;
  /* Preserve DEV bit; top 4 bits carry LBA[27:24]; keep LBA flag set */
  d->controller->lba4 = (d->controller->lba4 & DEVH_DEV) |
                        DEVH_LBA |
                        ((lba >> 24) & DEVH_HEAD);
...
int ide_attach_st (struct ide_controller *c, int drive, int fd)
{
  ...
  /* Initialise lba4 with LBA flag set */
  d->controller->lba4 = DEVH_LBA;
...
int ide_attach_hdf ( struct ide_controller *c, int drive, int fd )
{
  ...
  */
  ...
  d->LBA = d->cylinders * 512;

  //printf ("[IDE/HDL] Cylinders: %d Heads: %d Sectors: %d - filesize = %llu, lba = 0x%X\n", 
  //  d->cylinders, d->heads, d->sectors, file_size, d->LBA );
...
//printf ( "%s 1\n", __func__ );
  ...
  strncpy((char*)(p + 27), buf, 40);
	...
	put_le16(p + 49, (1 << 9)); /* LBA supported (always) */
	put_le16(p + 51, (240 << 8)); //0x200); /* PIO transfer cycle */
	put_le16(p + 52, 0x200); /* DMA transfer cycle */
	put_le16(p + 53, (1 << 1) | (1 << 0)); /* words 54-58 and 64-70 are valid */
...
printf ( "%s 3\n", __func__ );
  ...
  switch(type) 
  {
    ...
    case ACME_NEMESIS:
      ...
      break;
    case ACME_COYOTE:
      /* 20MB drive - now with LBA support */
      c = 615;
      h = 4;
      s = 16;
      ident[49] = le16(1 << 9); /* LBA */
