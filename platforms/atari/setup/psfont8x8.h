/*
 * psfont8x8.h - 8x8 monochrome font for the pre-boot setup page.
 *
 * Public domain. Daniel Hepper's font8x8 (github.com/dhepper/font8x8),
 * itself based on Marcel Sondaas's font8x8. Codepoints U+0000..U+007F,
 * one byte a scanline, top row first, BIT 7 = LEFTMOST pixel (Hepper's
 * original is bit 0 = leftmost; the bits are reversed here so a byte
 * drops straight onto an 8-pixel-aligned screen column).
 *
 * In 640x400 mono every scanline is drawn twice, giving an 8x16 cell, so
 * both screen modes lay out as the same 80x25 grid.
 */
#ifndef PSFONT8X8_H
#define PSFONT8X8_H
...
static const uint8_t ps_font8x8[128][8] = {
