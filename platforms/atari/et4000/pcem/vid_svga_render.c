...
void svga_render_blank(svga_t *svga) {
        ...
        for (x = 0; x < svga->hdisp; x++) {
                ...
                case 0:
                        for (xx = 0; xx < 9; xx++)
                                ((uint32_t *)buffer32->line[svga->displine])[(x * 9) + xx + 32] = 0;
                        break;
                case 1:
                        for (xx = 0; xx < 8; xx++)
                                ((uint32_t *)buffer32->line[svga->displine])[(x * 8) + xx + 32] = 0;
                        break;
                case 8:
                        for (xx = 0; xx < 18; xx++)
                                ((uint32_t *)buffer32->line[svga->displine])[(x * 18) + xx + 32] = 0;
                        break;
                case 9:
                        for (xx = 0; xx < 16; xx++)
                                ((uint32_t *)buffer32->line[svga->displine])[(x * 16) + xx + 32] = 0;
                        break;
                }
        }
}
        ...
        if (svga->fullchange) {
                ...
                for (x = 0; x < svga->hdisp; x += xinc) {
                        ...
                        if (x + xinc < svga->hdisp && (chr & (nextchr | svga->ksc5601_sbyte_mask) & 0x80)) {
                                if ((chr == svga->ksc5601_udc_area_msb[0] || chr == svga->ksc5601_udc_area_msb[1]) &&
                                    (nextchr > 0xa0 && nextchr < 0xff))
                                        dat = fontdatksc5601_user[(chr == svga->ksc5601_udc_area_msb[1] ? 96 : 0) +
                                                                  (nextchr & 0x7F) - 0x20][svga->sc];
                                ...
                                if ((svga->ksc5601_english_font_type >> 8) == 1)
                                        dat = fontdatksc5601[((svga->ksc5601_english_font_type & 0x7F) << 7) | (chr >> 1)]
                                                            [((chr & 1) << 4) | svga->sc];
                                else
                                        dat = svga->vram[charaddr + (svga->sc << 2)];
                        }
